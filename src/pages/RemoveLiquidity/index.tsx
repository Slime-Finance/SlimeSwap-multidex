...
export default function RemoveLiquidity({
  ...
  const { swapName, swapRouter} = useSwapRouter()
  const theme = useContext(ThemeContext)

  console.log("swaprouter",swapName,swapRouter);
  // burn state
  ...
  const [approval, approveCallback] = useApproveCallback(parsedAmounts[Field.LIQUIDITY])
  async function onAttemptToApprove() {
    ...
    console.log("pairContract",pairContract,nonce);
    const EIP712Domain = [
    ...
    const domain = {
      name: swapName === 'SLIME' ? 'Slime LPs' : 'Pancake LPs',
      version: '1',
      chainId,
  ...
  async function onRemove() {
    if (!library || !account) throw new Error('missing dependencies')
    const { [Field.CURRENCY_A]: currencyAmountA, [Field.CURRENCY_B]: currencyAmountB } = parsedAmounts
    if (!currencyAmountA || !currencyAmountB) {
      throw new Error('missing currency amounts')
    }
    const router = getRouterContract(swapName, swapRouter, library, account)
    ...
    console.log("Amount data ",liquidityAmount.raw.toString());
    ...
    console.log("finish");
    console.log(router);   console.log(args);
